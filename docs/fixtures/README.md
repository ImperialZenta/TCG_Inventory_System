...
Golden Mana Pool listing CSVs (`manapool-listing-staging-01.csv`, `manapool-upload-session-merged.csv`) are regenerated by the same script from staging-01 and staging-02 slices.
...
## Upload session set (Phase 5b — when CHL-* ships)

Metadata and golden CSVs for channel catalog / upload session tests. Generate via extended
[`scripts/generate-staging-fixtures.ts`](../../scripts/generate-staging-fixtures.ts) (spec only until implementation).

| File | Use for |
|------|---------|
| [`upload-session-staging-two-block.json`](upload-session-staging-two-block.json) | Session over staging-02 sealed blocks; complete payload |
| [`manapool-listing-staging-01.csv`](manapool-listing-staging-01.csv) | Golden single-block ManaBox export (PL-005 / **CHL-009**) |
| [`manapool-upload-session-merged.csv`](manapool-upload-session-merged.csv) | Golden cross-block qty merge (**CHL-004** / **CHL-009**) |
| [`upload-session-active-block-rejected.json`](upload-session-active-block-rejected.json) | Negative: ACTIVE block cannot join session |
| [`upload-session-pick-gating.json`](upload-session-pick-gating.json) | Order skips reserved SEALED block (**CHL-012**) — manual steps in [TESTING-PLAYBOOK Phase 4×5b](../TESTING-PLAYBOOK.md#phase-45b--pick-gating-chl-012--future-golden-path) |
| [`channel-catalog-mana-pool-bins.json`](channel-catalog-mana-pool-bins.json) | Bin membership for catalog filter UI (**CHL-008**) |

**Test helper (implementation):** `makeBlocksSealed()` in `tests/helpers/fixtures.ts` — formalize + seal without activate (opposite of `makeBlocksPickable()`).

Extend [`golden-path-inventory-map.json`](golden-path-inventory-map.json) with an `uploadSessions` section when goldens are generated.
