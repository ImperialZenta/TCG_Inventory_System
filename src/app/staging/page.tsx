...
export default async function StagingPage({ searchParams }: StagingPageProps) {
  ...
  const formalizedItems = await Promise.all(
    formalizedImports.map(async (item) => {
      const summary = await getImportUndoSummary(item.id);
      return {
        id: item.id,
        filename: item.filename,
        rowCount: item.rowCount,
        ...
        undoHint: summary.canUndo
          ? "All blocks unsealed — undo formalize on review page"
          : (summary.blockReason ?? "Undo not available"),
      };
    }),
  );
    ...
    <>
      ...
      {removedBlock && (
        <div className="mb-6 rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
          <p className="font-medium text-amber-200">Removed {removedBlock}</p>
          <p className="mt-1 text-amber-100/80">
            This block was linked to multiple staging imports. Open the relevant import below to
            review unassigned cards
            {Number.isFinite(cardsRemoved) && cardsRemoved > 0 && (
              <> ({cardsRemoved} card{cardsRemoved === 1 ? "" : "s"} freed)</>
            )}
            .
          </p>
        </div>
